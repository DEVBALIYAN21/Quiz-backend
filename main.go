...
type QuizDetails struct {
	...
	AttemptCount   int             `json:"attemptCount"`
	AverageScore   float64         `json:"averageScore"`
	StudentResults []StudentResult `json:"studentResults"`
}
...
func getQuizDetails(c *gin.Context) {
	...
	if quiz.HostedBy != userID {
		c.JSON(http.StatusForbidden, gin.H{"error": "Only the quiz owner can view detailed results"})
		return
	}
	...
	cursor, err = resultCollection.Find(ctx, bson.M{"quizid": quiz.ID})
	if err != nil {
	...
	studentResults := make([]StudentResult, 0, len(results))
	...
	c.JSON(http.StatusOK, quizDetails)
}

func getUserQuizzes(c *gin.Context) {
	userID := c.MustGet("userId").(primitive.ObjectID)

	var params UserQuizzesParams
	...
	quizDetailsList := make([]QuizDetails, 0, len(quizzes))
	for _, quiz := range quizzes {
		...
		cursor, err = resultCollection.Find(ctx, bson.M{"quizid": quiz.ID})
		if err != nil {
			log.Printf("Failed to fetch results for quiz %s: %v", quiz.ID.Hex(), err)
			continue
		}
		if err = cursor.All(ctx, &results); err != nil {
			log.Printf("Failed to decode results for quiz %s: %v", quiz.ID.Hex(), err)
			continue
		}
	...
	total, err := quizCollection.CountDocuments(ctx, bson.M{"hostedby": userID})
	if err != nil {
